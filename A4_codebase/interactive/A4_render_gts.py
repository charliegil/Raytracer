...
def main():
    ...
    for rr_prob in rr_probabilities:
        for bounce in bounces:
            renderer.reset()
            renderer.max_bounces[None] = bounce
            renderer.rr_termination_probability[None] = rr_prob

            for _ in tqdm(range(spp), desc="Rendering Explicit Path Tracing Image @ {N} bounces x {s} SPP with Russian Roulette probability {rr}".format(N=bounce, s=spp, rr=rr_prob)):
                renderer.render()
            renderer.postprocess()
